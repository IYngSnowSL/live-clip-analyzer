...
> 由 AI 生成代码时，请在使用前充分理解其行为并自行测试验证。
...
→ 场景精炼 → 弹幕统计 → 全局抽帧 → ASR 转写 → 画面理解
→ 逐段 LLM 分析 → 综合评分 → 候选生成 → 报告 + JSON 导出
```
...
下一步计划把切分主轴从"画面变化"转为"**字幕语义**"：对完整视频做全量语音转写，
整理字幕文档，按内容一段一段切出"话题段"（导航层）并在其中细切
"切片友好片段"（30 秒~3 分钟，可直接发布）；弹幕、画面降为辅助信息。
完整问题清单、能力盘点与决策记录见上方的设计文档。
