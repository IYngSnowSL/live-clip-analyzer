...
一款**本地直播录像切片分析工具**，核心目的：

> **对没有完整看完的直播录播做分析，快速找到"从时间到时间"的、
> 具有主要话题或有趣内容的片段。**

输入一段直播录播视频（FLV / MP4）和可选的 B 站弹幕 XML 文件，全自动完成
字幕语义切分、场景切分、弹幕统计、语音转写、画面理解与逐段 AI 分析，最终输出：

- 🗂️ **话题目录**：整场直播切成"XX 局游戏""聊天互动"这样的一段段话题，快速了解全程都聊了什么
- ✂️ **长切片候选**：有趣内容的完整片段（1~5 分钟），点击时间段即跳转视频预览
- 💬 **单句素材候选**：适合二创的原话引用，自带时间戳
- 📋 **详细时间轴**（中英双语标题、概述、画面、语音、弹幕、评分）
